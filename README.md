...
- 中心主视觉区域为“中心态势星系”，以银河系式动态关系图展示区域、图表与健康状态，悬浮时会柔和放大并产生云雾扩散和空间聚焦效果。
- 仪表盘布局调整为三行网格，中心星系纵向贯穿主视觉区域，趋势、排行、占比、分布和系统状态围绕中心展开。
- 排行榜与系统日志压缩为更稳定的等高列表，避免内容挤压导致布局跳动。
- 周围信息面板在中心主框聚焦时会轻微弱化，突出主图表阅读层级。
- 已适配 `prefers-reduced-motion`，系统开启减少动态效果时会自动降低动画强度。
- 本地运行后将鼠标悬浮到指标卡、图表面板和中心态势星系，即可预览动效。
